...
pub fn read_u8(addr: usize) -> u8 {
    // Safety: `addr` must be a valid MMIO address for a u8 register.
...
pub fn read_u8_masked(addr: usize, mask: u8) -> u8 {
    // Safety: `addr` must be a valid MMIO address for a u8 register.
    unsafe { ptr::read_volatile(addr as *const u8) & mask }
}
...
pub fn write_u16(addr: usize, val: u16) {
    // Safety: `addr` must be a valid MMIO address for a u16 register.
...
pub fn mask_u32p(ptr: *mut u32, mask: u32) {
    let r = unsafe { ptr::read_volatile(ptr) };
    unsafe { ptr::write_volatile(ptr, r | mask) }
}
...
pub fn unmask_u32p(ptr: *mut u32, unmask: u32) {
    let r = unsafe { ptr::read_volatile(ptr) };
    unsafe { ptr::write_volatile(ptr, r & !unmask) }
}
...
pub fn mask_u8(addr: usize, mask: u8) {
    // Safety: `addr` must be a valid MMIO address for a u8 register.
    let r = unsafe { ptr::read_volatile(addr as *const u8) };
    unsafe { ptr::write_volatile(addr as *mut u8, r | mask) }
}
...
pub fn unmask_u8(addr: usize, unmask: u8) {
    // Safety: `addr` must be a valid MMIO address for a u8 register.
    let r = unsafe { ptr::read_volatile(addr as *const u8) };
    unsafe { ptr::write_volatile(addr as *mut u8, r & !unmask) }
}
