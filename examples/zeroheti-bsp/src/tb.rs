...
pub fn rtl_tb_signal_fail() {
    ...
    crate::mmio::write_u32(0x380, OK_BIT | FAIL_BIT);
}
...
pub fn rtl_tb_signal_ok() {
    ...
    const OK_BIT: u32 = 0b1 << 31;
    crate::mmio::write_u32(0x380, OK_BIT);
}
...
fn blink_exception(_trap_frame: &riscv_rt::TrapFrame) -> ! {
    use crate::{
        NOPS_PER_SEC, asm_delay,
        led::{Led, led_off, led_set},
        sprintln,
    };
...
#[cfg(feature = "fpga")]
fn fail_blink() -> ! {
    use crate::led::{Led, led_off, led_on};
    ...
    match () {
        ...
        #[cfg(not(feature = "rtl-tb"))]
        () => {
            use crate::{NOPS_PER_SEC, asm_delay};
            loop {
                led_on(Led::Ld0);
                led_on(Led::Ld1);
                led_on(Led::Ld2);
