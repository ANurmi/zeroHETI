/* Author: Henri Lunnikivi <henri.lunnikivi@tuni.fi> */

ENTRY(_start)

MEMORY
{
  IMEM (rx ) : ORIGIN = 0x10000, LENGTH = 0x10000
  DMEM (rwx) : ORIGIN = 0x20000, LENGTH = 0x08000
}

/* Regions are setup like in link.ld for rt-ss written by Antti Nurmi. I didn't put any more thought into it. */

REGION_ALIAS("REGION_TEXT", IMEM);

REGION_ALIAS("REGION_RODATA", DMEM);
REGION_ALIAS("REGION_BSS", DMEM);
REGION_ALIAS("REGION_HEAP", DMEM);
REGION_ALIAS("REGION_STACK", DMEM);
REGION_ALIAS("REGION_DATA", DMEM);

/* Default interrupt trap entry point. When vectored trap mode is enabled,
   the riscv-rt crate provides an implementation of this function, which saves caller saved
   registers, calls the the DefaultHandler ISR, restores caller saved registers and returns.
*/
/*
PROVIDE(_start_DefaultHandler_trap = _start_trap);
*/

/* Core interrupts (NC: not connected) */
/* NC: PROVIDE(_start_SupervisorSoft_trap = _start_DefaultHandler_trap); */
PROVIDE(_start_MachineSoft_trap = _start_DefaultHandler_trap);
/* NC: PROVIDE(_start_SupervisorTimer_trap = _start_DefaultHandler_trap); */
PROVIDE(_start_MachineTimer_trap = _start_DefaultHandler_trap);
/* NC: PROVIDE(_start_SupervisorExternal_trap = _start_DefaultHandler_trap); */
PROVIDE(_start_MachineExternal_trap = _start_DefaultHandler_trap);

/* The following are specific to zeroHETI */
PROVIDE(_start_Timer0Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer0Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer1Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer1Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer2Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer2Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer3Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer3Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer4Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer4Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer5Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer5Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer6Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer6Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer7Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer7Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer8Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer8Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer9Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer9Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer10Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer10Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer11Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer11Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer12Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer12Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer13Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer13Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer14Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer14Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer15Ovf_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Timer15Cmp_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Uart_trap = _start_DefaultHandler_trap);
PROVIDE(_start_I2c0_trap = _start_DefaultHandler_trap);
PROVIDE(_start_I2c1_trap = _start_DefaultHandler_trap);
PROVIDE(_start_TqFull_trap = _start_DefaultHandler_trap);
PROVIDE(_start_TqNFull_trap = _start_DefaultHandler_trap);
PROVIDE(_start_SpiEvent0_trap = _start_DefaultHandler_trap);
PROVIDE(_start_SpiEvent1_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Mbx_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Ext0_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Ext1_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Ext2_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Ext3_trap = _start_DefaultHandler_trap);
PROVIDE(_start_Nmi_trap = _start_DefaultHandler_trap);
