...
static inline void edfic_set_prio(uint32_t irq, uint32_t prio)
{
    uint32_t flags = sys_read32(EDFIC_LINE(irq)) & 0xFFU;
    sys_write32(flags | (TO_DL(prio) << 8), EDFIC_LINE(irq));
}
...
static void isr_upd(const void *arg)
{
    ...
    edfic_set_prio(IRQ_TIMER_CMP(n), TO_PRIO(nperiod / 2));
