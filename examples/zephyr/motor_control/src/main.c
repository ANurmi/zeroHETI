...
void inline mbx_report_speed(uint8_t idx, uint32_t speed)
{
...
uint32_t inline motor_read_speed(uint8_t idx)
{
    uint8_t buf[4] = {0};
...
void inline motor_write_ctrl(uint8_t idx, uint8_t voltage)
{
...
void inline motor_write_tune(uint8_t idx, int16_t tune)
{
    uint8_t buf[2] = {
        (uint8_t)(tune & 0xff),
        (uint8_t)(tune >> 8)
