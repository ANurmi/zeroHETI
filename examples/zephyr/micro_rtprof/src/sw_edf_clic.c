...
int main(void)
{
	...
	if (is_edfic) {
		printf("ERROR: this arm needs a CLIC build. Rebuild with: make verilate INTC=CLIC\n");
	}

	// How many spin() iterations make up each task's C
	{
	...
	// Setup IRQ
	IRQ_CONNECT(IRQ_TIMER_CMP(0), PRIO_T0, isr_release, (void *)0, 1);
	IRQ_CONNECT(IRQ_TIMER_CMP(1), PRIO_T1, isr_release, (void *)1, 1);
	IRQ_CONNECT(IRQ_TIMER_CMP(2), PRIO_T2, isr_release, (void *)2, 1);
	IRQ_CONNECT(IRQ_TIMER_CMP(3), PRIO_FINISH, isr_finish, NULL, 1);
	...
	for (int i = 0; i < NUM_TASKS; i++) {
		riscv_clic_irq_vector_set(IRQ_TIMER_CMP(i));
		irq_enable(IRQ_TIMER_CMP(i));
	}
	riscv_clic_irq_vector_set(FINISH_IRQ);
	irq_enable(FINISH_IRQ);
