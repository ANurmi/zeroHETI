...
compile_error!("this impl only for EDFIC, use -Fintc-edfic");
...
mod app {
    ...
    fn init() -> Shared {
        ...
        sprintln!(
            ...
            SIM_PARAMS.mbx_dl_us,
            SIM_PARAMS.wrn_dl_us,
            SIM_PARAMS.rep_dl_us,
        );
        sprintln!(
            "\x1b[33mWARN\x1b[0m: deadlines are hard-coded in the EDFIC version. If deadlines change, a code review is required."
        );

        // HACK: use mtimer to start the sim, CLIC cannot enable IRQ after pend,
        // see: https://github.com/ANurmi/zeroHETI/issues/42
        // It takes about 130 us for the rest of setup to complete in the EDFIC version.
        // Let's leave a little margin.
    ...
    #[task(binds = Timer0Cmp, priority = 3000, shared = [i2c, mbx])]
    struct ReadM0 {
        speed_real: u32,
    }
    ...
    #[task(binds = Timer1Cmp, priority = 3000, shared = [i2c, mbx])]
    struct ReadM1 {
        speed_real: u32,
    }
    ...
    #[task(binds = Timer2Cmp, priority = 3000, shared = [i2c, mbx])]
    struct ReadM2 {
        speed_real: u32,
    }
    ...
    #[task(binds = Timer3Cmp, priority = 3000, shared = [i2c, mbx])]
    struct ReadM3 {
        speed_real: u32,
    }
    ...
    #[task(binds = Mbx, priority = 5000, shared = [i2c, v_tgt0, v_tgt1, v_tgt2, v_tgt3])]
    struct GetMail;
    ...
    #[task(binds = Ext0, priority = 4000, shared = [i2c, v_tgt0])]
    struct TuneM0;
    ...
    #[task(binds = Ext1, priority = 4000, shared = [i2c, v_tgt1])]
    struct TuneM1;
    ...
    #[task(binds = Ext2, priority = 4000, shared = [i2c, v_tgt2])]
    struct TuneM2;
    ...
    #[task(binds = Ext3, priority = 4000, shared = [i2c, v_tgt3])]
    struct TuneM3;
