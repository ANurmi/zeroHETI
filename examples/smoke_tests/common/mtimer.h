...
static inline void start_mtimer() {
  set_bit_apb(MTIMER_CTRL, EN_OFFS);
}

static inline void stop_mtimer() {
  clear_bit_apb(MTIMER_CTRL, EN_OFFS);
}

static inline void set_mtimer_cmp(uint64_t cmp) {
...
static inline void set_mtimer_prescaler(uint8_t value) {
