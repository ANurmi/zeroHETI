...
//! that wiring: a fixed-size, lock-free, timestamped buffer. Applications use
//! [`define_observer`] to supply the generated app module; the macro provides
//! the marker type, RTIC hook implementation, and name-aware dump function.
//!
//! # Concurrency model
//!
...
/// Defines an RTIC observability marker and its trace dump for an application.
///
/// The application module is generated by RTIC and contains the
/// `RticObservability`, `TaskId`, `ResourceId`, `task_name`, and `res_name`
/// items. Pass the application module identifier, for example
...
macro_rules! define_observer {
    ($app:ident) => {
        ...
        impl $app::RticObservability for Obs {
            fn on_task_act(task: $app::TaskId, task_prio: u16) {
                $crate::obs_push($crate::ObsKind::Act, task as u8, task_prio, 0);
            }

            fn on_task_comp(task: $app::TaskId, task_prio: u16) {
                $crate::obs_push($crate::ObsKind::Comp, task as u8, task_prio, 0);
            }

            fn on_res_acq(res: $app::ResourceId, task_prio: u16, ceiling: u16) {
                $crate::obs_push($crate::ObsKind::Acq, res as u8, task_prio, ceiling);
            }

            fn on_res_rel(res: $app::ResourceId, task_prio: u16, ceiling: u16) {
                $crate::obs_push($crate::ObsKind::Rel, res as u8, task_prio, ceiling);
            }
        }

        pub fn obs_dump(ts_unit: $crate::TsUnit) {
            $crate::obs_dump($app::task_name, $app::res_name, ts_unit);
        }
    };
}
