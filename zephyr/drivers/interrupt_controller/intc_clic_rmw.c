...
#include "intc_clic_rmw.h"

#define DT_DRV_COMPAT riscv_zeroheti_clic
...
static inline void clic_write_byte_rmw(const struct device *dev,
                                       uint32_t irq,
                                       uint32_t byte_index,
                                       uint8_t value)
{
    const uint32_t slot  = CLIC_CTRL(irq);
    const uint32_t shift = byte_index * 8U;
    const uint32_t mask  = 0xFFu << shift;

    uint32_t reg = read_clic32(dev, slot);
    reg &= ~mask;
    reg |= ((uint32_t)value << shift);

    write_clic32(dev, slot, reg);
}

static inline uint8_t clic_read_byte_from_slot(const struct device *dev,
                                               uint32_t irq,
                                               uint32_t byte_index)
{
    const uint32_t slot  = CLIC_CTRL(irq);
    ...
    return (uint8_t)((reg >> shift) & 0xFFu);
}
...
{
	if (IS_ENABLED(CONFIG_ZEROHETI_LEGACY_CLIC_MEMORYMAP_ACCESS)) {
		const struct device *dev = DEVICE_DT_INST_GET(0);

		clic_write_byte_rmw(dev, irq, 1, 0x01);
	} else {
...
int riscv_clic_irq_is_enabled(uint32_t irq)
{
	...
	if (IS_ENABLED(CONFIG_ZEROHETI_LEGACY_CLIC_MEMORYMAP_ACCESS)) {
		...
		uint32_t v = read_clic32(dev, CLIC_CTRL(irq)); 
        ie_byte = (uint8_t)((v >> 8) & 0xFF);  
...
void riscv_clic_irq_priority_set(uint32_t irq, uint32_t pri, uint32_t flags)
{
    ...
    uint8_t intctrl =
        (MIN(pri, max_prio)  << (8U - data->intctlbits)) |
        (MIN(pri, max_level) << (8U - data->nlbits)) |
        BIT_MASK(8U - data->intctlbits);

    if (IS_ENABLED(CONFIG_ZEROHETI_LEGACY_CLIC_MEMORYMAP_ACCESS)) {
        /* INTCTRL is byte3 of the 32-bit slot */
        clic_write_byte_rmw(dev, irq, 3, intctrl);
    } else {
        ...
        clicintctl |= (uint32_t)intctrl << bit_offset;
	...
	if (IS_ENABLED(CONFIG_ZEROHETI_LEGACY_CLIC_MEMORYMAP_ACCESS)) {
		clic_write_byte_rmw(dev, irq, 2, clicattr.w);
	} else {
...
void riscv_clic_irq_vector_set(uint32_t irq)
{
	if (IS_ENABLED(CONFIG_ZEROHETI_LEGACY_CLIC_MEMORYMAP_ACCESS)) {
		const struct device *dev = DEVICE_DT_INST_GET(0);
        uint8_t attr = clic_read_byte_from_slot(dev, irq, 2);
        union CLICINTATTR clicattr = {.w = attr};
        clicattr.b.shv = 1;
        clic_write_byte_rmw(dev, irq, 2, clicattr.w);
	} else {
		uint32_t clicintattr, bit_offset = 8 * (irq % 4);
...
void riscv_clic_irq_set_pending(uint32_t irq)
{
	if (IS_ENABLED(CONFIG_ZEROHETI_LEGACY_CLIC_MEMORYMAP_ACCESS)) {
		const struct device *dev = DEVICE_DT_INST_GET(0);
		clic_write_byte_rmw(dev, irq, 0, 0x01);
	} else {
...
static int clic_init(const struct device *dev)
{
	...
	if (IS_ENABLED(CONFIG_ZEROHETI_CLIC_SMCLICCONFIG_EXT)) {
		if (IS_ENABLED(CONFIG_ZEROHETI_LEGACY_CLIC_MEMORYMAP_ACCESS)) {
...
#define CLIC_INTC_DATA_INIT(n)                                                                     \
	static struct clic_data clic_data_##n = {                                                  \
		.nlbits = CONFIG_ZEROHETI_CLIC_PARAMETER_MNLBITS,                                           \
		.intctlbits = CONFIG_ZEROHETI_CLIC_PARAMETER_INTCTLBITS,                                    \
	};
#define CLIC_INTC_CONFIG_INIT(n)                                                                   \
	const static struct clic_config clic_config_##n = {                                        \
		.base = COND_CODE_1(CONFIG_ZEROHETI_LEGACY_CLIC_MEMORYMAP_ACCESS,                           \
